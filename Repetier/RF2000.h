...
/**  Specifies the maximal drive over millimeters which the z-endstop can bear without getting damaged or degraded */
/*
...
/**  Allows to use the device for milling */
#ifndef FEATURE_MILLING_MODE
#define FEATURE_MILLING_MODE 0 // 1 = on, 0 = off
#endif
#if FEATURE_MILLING_MODE

/**  Enables automatic compensation in z direction for the operationg mode "mill" */
#define FEATURE_WORK_PART_Z_COMPENSATION 1 // 1 = on, 0 = off

/**  This feature allows to move the milling bed upwards automatically until the miller is hit. The found position is taken over as Z=0 automatically.
...
/**  Enables/disables the menu entry which allows to choose the currently installed miller type */
...
/**  Define lower acceleration to reach very small speeds */
...
/**  Allows to use the 230V output */
#define FEATURE_230V_OUTPUT 1 // 1 = on, 0 = off

/**  Allows to use the RGB light effects */
#define FEATURE_RGB_LIGHT_EFFECTS 1 // 1 = on, 0 = off

/**
 * Allows to use the 24V FET outputs
...
/**  This feature enables a security check so that the Firmware for a RF2000 is only working on a RF2000 Board otherwise the firmware beeps. */
...
/**  The maximum value, I-gain can contribute to the output. Overridden if EEPROM activated. */
#define EXT0_PID_INTEGRAL_DRIVE_MAX HT2_PID_INTEGRAL_DRIVE_MAX
/**  lower value for integral part. Overridden if EEPROM activated. */
...
/**  Faktor for the advance algorithm. 0 disables the algorithm.  Overridden if EEPROM activated.
...
/**  Units (mm/inches) to retract filament when extruder is heating up. Overridden if EEPROM activated. Set
to 0 to disable. */
...
/**  Maximal temperature which can be set for the heating bed */
...
/**  Allows to choose whether the setpoint and the current value of the heat bed temperature shall be compensated so that the temperature offset which is caused by the printing plate is reduced */
#define FEATURE_HEAT_BED_TEMP_COMPENSATION 1 // 1 = on, 0 = off

#if FEATURE_HEAT_BED_TEMP_COMPENSATION
/**  The following vlaue must be NumberOfTemperatures -1 */
#define BED_TEMP_COMPENSATION_INDEX_MAX 5
/**  The following values represent the setpoint temperatures */
#define BED_SETPOINT_TEMPERATURES \
    { 60, 80, 100, 120, 140, 160 }
/**  The following values represent the real temperature which is measured at the surface of the printing bed in case the temperature sensor delivers the setpoint temperatures */
...
/**  Analog pin of analog sensor to read temperature of the RF2000's optional sensor port.  */
...
/**  Define the type Z-Endstop-Installation */
...
/**  Motor Current settings at start: Tweak with menu for better silence <-> stability
...
/**  Specifies whether the firmware shall wait a short time after turning on of the stepper motors - this shall avoid that the first steps are sent to the stepper before it is ready */
...
/**  Automatic filament change, unmounting of the filament - ensure that G1 does not attempt to extrude more than EXTRUDE_MAXLENGTH */
...
/**  Automatic filament mount, mounting of the filament with heating. Stop @ XXXX digits */
...
/**  Heaters: speed of the PWM signal,          0 = 15.25Hz, 1 = 30.51Hz, 2 = 61.03Hz */
#define HEATER_PWM_SPEED 1

/**  Extruder Coolers: speed of the PWM signal, 0 = 15.25Hz, 1 = 30.51Hz, 2 = 61.03Hz */
...
/**  Part Fan:                                  1 = 15.3Hz,  2 = 7.62Hz, 3 = 5.1Hz, 4 = 3.82Hz,  5 = 3.06Hz,  6 = 2.55Hz, 7 = 2.1Hz */
#define PART_FAN_DEFAULT_PWM_SPEED_DIVISOR 3
/**  the following values can be used in order to fine-tune the operating range of the cooler: */
...
/**  use PDM instead of PWM for part fan */
...
/**
 The firmware can only handle a limited interrupt frequency cleanly.
...
/**  Specifies if you want to adjust minimal and maximum compensation steps to first layer */
#define AUTOADJUST_STARTMADEN_AUSSCHLUSS 0.352f

/**  Specifies until which height the z compensation must complete */
#define HEAT_BED_Z_COMPENSATION_MAX_MM 10 // [mm]

/**  Specifies from which height on the z compensation shall be performed
...
/**  Configuration of the heat bed scan */
#if NUM_EXTRUDER == 2
...
/**  Specifies the maximal static z-offset which can be configured.
*/
#define WORK_PART_MAX_STATIC_Z_OFFSET_MM 10 // [mm]

/**  Configuration of the work part scan */
...
/**  Configuration of the pause steps */
...
/**  The following commands are executed before the z-origin is set to 0. */
...
/**  Enables debug outputs from the work part scan */
#define DEBUG_WORK_PART_SCAN 0 // 1 = on, 0 = off

#if FEATURE_FIND_AXIS_ORIGIN

/**  Enables debug outputs from the search of the z-origin */
...
/**  Set the 24V FET output defaults  */
...
/**  Specfies the default RGB light mode */
#define RGB_LIGHT_DEFAULT_MODE RGB_MODE_WHITE

/**  Specifies the time interval after which the RGB light status switches from idle to color change */
#define RGB_LIGHT_COLOR_CHANGE_DELAY 30 // [s]

/**  Specfies the temperature tolerance for the switching of the RGB light */
#define RGB_LIGHT_TEMP_TOLERANCE 5.0 // [°C]

/**  Specifies the color for heating up */
